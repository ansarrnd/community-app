...
import { DEMO_AUTH_CREDENTIALS } from '../infrastructure/config/demoAuthCredentials';
...
async function seedAuthEmulatorUsers() {
  const admin = await import('firebase-admin');
  const auth = admin.auth();
  const db = admin.firestore();

  for (const cred of DEMO_AUTH_CREDENTIALS) {
    try {
      ...
      console.log(`[Seed] Created Auth user ${cred.email}`);
    } catch (err: unknown) {
      const code = (err as { code?: string }).code;
      if (code !== 'auth/uid-already-exists' && code !== 'auth/email-already-exists') {
        throw err;
      }
      console.log(`[Seed] Auth user ${cred.email} already exists — updating claims/profile`);
    }

    await auth.setCustomUserClaims(cred.uid, { role: cred.role });
    await db.collection('users').doc(cred.uid).set(
      {
  ...
  console.log(`[Seed] Auth emulator seeded with ${DEMO_AUTH_CREDENTIALS.length} users + custom claims`);
}
...
async function main() {
  ...
  if (process.env.FIRESTORE_EMULATOR_HOST) {
    ...
    if (process.env.FIREBASE_AUTH_EMULATOR_HOST) {
      await seedAuthEmulatorUsers();
    } else {
      console.log('[Seed] Tip: set FIREBASE_AUTH_EMULATOR_HOST=localhost:9099 to seed Auth users + claims');
    }
  } else {
    await seedMock();
